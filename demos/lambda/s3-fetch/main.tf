...
# API Gateway resource for file path (proxy+)
# resource "aws_api_gateway_resource" "s3_fetch_proxy" {
#   rest_api_id = var.parent_api_id
...
#   rest_api_id   = var.parent_api_id
#   resource_id   = aws_api_gateway_resource.s3_fetch_proxy.id
#   http_method   = "GET"
#   authorization = "NONE"
# }
...
#   source_arn = "arn:aws:execute-api:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.parent_api_id}/*/${aws_api_gateway_method.s3_fetch_get.http_method}${aws_api_gateway_resource.s3_fetch_proxy.path}"
# }
